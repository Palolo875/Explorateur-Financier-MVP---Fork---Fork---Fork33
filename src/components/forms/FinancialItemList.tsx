...
function EditableItem({ item, type, onSave, onCancel }: EditableItemProps) {
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="flex items-center">
          <input
            ...
            className="bg-black/30 border border-white/10 rounded-lg py-2 px-3 w-32 text-white mr-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            placeholder="Montant"
            min="0.01"
            step="0.01"
            required
        ...
        <input
          ...
          className="w-full bg-black/30 border border-white/10 rounded-lg py-2 px-3 text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
          ...
          return (
            ...
            >
              {isEditing ? (
                ...
                <div className="bg-black/20 p-4 rounded-lg border border-white/10 hover:border-white/20 transition-colors">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center mb-2">
                        ...
                        <span className="text-sm text-gray-400">€</span>
                        {item.isRecurring && (
                          <span className="ml-2 px-2 py-1 bg-indigo-600/20 text-indigo-300 text-xs rounded-full">
                            Récurrent
                          </span>
                        )}
                      </div>
                    ...
                    <div className="relative">
                      <button
                        onClick={() => setExpandedMenu(isMenuExpanded ? null : item.id!)}
                            ...
                            <div className="py-1">
                              ...
                              {onMove && (
                                <>
                                  ...
                                  {(['incomes', 'expenses', 'savings', 'debts'] as FinancialDataType[])
                                    .filter(t => t !== type)
                                    .map(targetType => (
                                      <button
                                        key={targetType}
                                        onClick={() => handleMove(item.id!, targetType)}
                                        className="w-full flex items-center px-4 py-2 text-sm text-white hover:bg-white/10 transition-colors"
                                      >
                                        <MoveIcon className="h-4 w-4 mr-2" />
                                        {targetType === 'incomes' ? 'Revenus' : 
                                         targetType === 'expenses' ? 'Dépenses' : 
                                         targetType === 'savings' ? 'Épargne' : 'Dettes'}
                                      </button>
                                    ))
                                  }
                                </>
