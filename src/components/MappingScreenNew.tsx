...
import { PlusIcon, ArrowRightIcon, HeartIcon, BrainIcon, PencilIcon, SaveIcon, BarChart3Icon, ArrowUpIcon, ArrowDownIcon, DownloadIcon, UploadIcon, RotateCcwIcon, TagIcon } from 'lucide-react';
...
export function MappingScreenNew() {
  ...
  useEffect(() => {
    ...
    // Tags temporels
    if (hour < 10) tags.push('Matin');else if (hour < 14) tags.push('Midi');else if (hour < 18) tags.push('Après-midi');else tags.push('Soir');
    if (day === 0 || day === 6) tags.push('Weekend');else tags.push('Semaine');
    ...
    // Tags émotionnels
    if (emotionalContext) {
      if (emotionalContext.mood > 7) tags.push('Stressé');else if (emotionalContext.mood < 4) tags.push('Détendu');
  ...
  const handleSaveJournal = useCallback(async () => {
    ...
    try {
      const entryId = await dataPersistence.saveJournalEntry(journalEntry, selectedTags, emotionalContext?.mood);
  ...
  return <div className="w-full max-w-6xl mx-auto pb-20">
      ...
      <GlassCard className="p-6 mb-6" animate>
        ...
        <div className="mt-4 flex justify-end">
          <button onClick={() => setShowJournal(!showJournal)} className={`flex items-center text-sm px-3 py-1.5 rounded-lg transition-colors ${showJournal ? 'bg-indigo-600' : 'bg-black/30 hover:bg-black/40'}`}>
        ...
        <AnimatePresence>
          {showJournal && <motion.div initial={{
          opacity: 0,
          height: 0
        }} animate={{
          opacity: 1,
          ...
          duration: 0.2
              ...
              <div className="mt-4 bg-black/20 p-4 rounded-lg">
                ...
                <textarea value={journalEntry} onChange={e => setJournalEntry(e.target.value)} placeholder="Comment vous sentez-vous par rapport à votre situation financière actuelle ?" className="w-full bg-black/30 border border-white/10 rounded-lg p-3 text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 min-h-[100px] resize-none" maxLength={1000} />
                <div className="flex justify-between items-center mt-2">
                  <span className="text-xs text-gray-400">{journalEntry.length}/1000</span>
                  <button onClick={handleSaveJournal} className="flex items-center bg-indigo-600 hover:bg-indigo-700 px-3 py-1.5 rounded-lg text-sm transition-colors">
      ...
      <GlassCard className="p-6 mb-6" animate>
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="flex items-center mb-2">
              <div className="w-full bg-black/20 h-4 rounded-full mr-4">
                <div className={`h-4 rounded-full bg-gradient-to-r transition-all duration-1000 ${healthScore > 70 ? 'from-green-500 to-green-300' : healthScore > 50 ? 'from-yellow-500 to-yellow-300' : 'from-red-500 to-red-300'}`} style={{
                width: `${healthScore}%`
              }} />
              </div>
          ...
          <div className="flex flex-col gap-2">
            <button onClick={() => setShowInsights(!showInsights)} className={`flex items-center text-sm px-3 py-1.5 rounded-lg transition-colors ${showInsights ? 'bg-indigo-600' : 'bg-black/30 hover:bg-black/40'}`}>
              <BrainIcon className="h-4 w-4 mr-1" />
              Insights
            </button>
            
            <div className="flex gap-1">
              <button onClick={handleExportData} className="p-1.5 bg-black/30 hover:bg-black/40 rounded-lg transition-colors" title="Exporter">
              ...
              <button onClick={handleClearAllData} className="p-1.5 bg-red-600/30 hover:bg-red-600/50 rounded-lg transition-colors" title="Effacer tout">
        ...
        <AnimatePresence>
          {showInsights && <motion.div initial={{
          opacity: 0,
          height: 0
        }} animate={{
          opacity: 1,
          ...
          duration: 0.2
              ...
              <div className="mt-4 bg-black/20 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  {insights.map((insight, index) => <div key={index} className="flex items-start">
      ...
      <GlassCard className="p-6 mb-6" animate>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {TABS.map(tab => <div key={tab.key} className="bg-black/20 p-4 rounded-lg">
        ...
        <div className="bg-black/20 p-4 rounded-lg">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center">
              {totals.balance >= 0 ? <ArrowUpIcon className="h-6 w-6 text-green-400 mr-2" /> : <ArrowDownIcon className="h-6 w-6 text-red-400 mr-2" />}
      ...
      <GlassCard className="p-6 mb-6" animate>
        ...
        <div className="flex flex-wrap border-b border-white/10 mb-6">
          {TABS.map(tab => <button key={tab.key} className={`mr-6 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.key ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-300'}`} onClick={() => handleTabChange(tab.key)}>
              <span className="mr-2">{tab.icon}</span>
              {tab.label}
              {financialData[tab.key].length > 0 && <span className="ml-2 bg-indigo-600/20 text-indigo-300 px-2 py-0.5 rounded-full text-xs">
        ...
        {/* Liste des éléments */}
        <FinancialItemList type={activeTab} items={financialData[activeTab]} onEdit={handleEditSubmit} onDelete={handleDelete} onDuplicate={handleDuplicate} onMove={handleMove} />

        {/* Bouton d'ajout */}
        <div className="mt-6 flex justify-center">
          <button onClick={() => setIsAdding(!isAdding)} className={`flex items-center justify-center px-6 py-3 rounded-lg font-medium transition-colors ${isAdding ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-indigo-600 hover:bg-indigo-700 text-white'}`}>
        ...
        {/* Formulaire d'ajout */}
        <AnimatePresence>
          {isAdding && <div className="mt-6">
              <FinancialItemForm type={activeTab} onSubmit={handleAddSubmit} onCancel={() => setIsAdding(false)} availableTags={availableTags} selectedTags={selectedTags} onTagChange={setSelectedTags} />
            </div>}
        </AnimatePresence>
      </GlassCard>
      ...
      <div className="flex justify-end">
        <button onClick={handleContinue} className="btn btn-primary flex items-center justify-center px-8 py-3 text-lg">
